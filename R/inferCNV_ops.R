...
run <- function(
    ...
    # or set to a specific value or "auto", or NA to turn off
    ...
    ## only meanvar supported, others experimental
    #sim_method=c('meanvar', 'simple', 'splatter'),
    sim_method = 'meanvar',
    ...
    # for filtering and HMM
    analysis_mode = c('subclusters', 'samples', 'cells'),
    tumor_subcluster_partition_method = c(
  ...
  if (HMM && HMM_type == 'i6' && smooth_method == 'coordinates') {
    flog.error(paste0(
      "Cannot use 'coordinate' smoothing method with ",
      "i6 HMM model at this time."
    ))
  ...
  if (smooth_method == 'coordinates' && window_length < 10000) {
    ...
    flog.warn(paste0(
      "smooth_method set to 'coordinates', but window_length is less than ",
      "10.000, setting it to 10.000.000. Please set a different value ",
      "> 10.000 if this default does not seem suitable."
    ))
  }
  ...
  if (analysis_mode == "cells" && HMM_report_by != "cell") {
    flog.warn(paste0(
      "analysis_mode is \"cells\" but HMM_report_by is not, ",
      "changing HMM_report_by to \"cells\"."
    ))
    HMM_report_by <- "cell"
  }
  tumor_subcluster_partition_method <- match.arg(
  ...
  ignored <- which(arg_names == "" | arg_names == "infercnv_obj")
  arg_names <- arg_names[- ignored]
  # evaluate variables such as output_dir
  for (n in arg_names) {
    call_match[[n]] <- get(n)
  }

  # add the arguments that are checked against a list of allowed values and
  # autofilled with the first if none is given using match.arg()
  ...
  ignored <- which(names(call_match) == "" | names(call_match) == "infercnv_obj")
  current_args <- as.list(call_match[- ignored])
  # add the arguments that are checked against a list of allowed values and
  # autofilled with the first if none is given using match.arg()
  ...
  if (resume_mode) {
    ...
    for (i in rev(seq_along(reload_info$expected_file_names))) {
      ...
      # step 17 appears in two conditions because if last found step is 18,
      # step 19 requires the results of both step 17 and 18 to run
      chk.step <- (i == 20) ||
        (i %in% seq(17, 19) && skip_hmm == 0) ||
        (i == 17 && skip_hmm == 2)
      if (chk.step) {
        if (i == 18 && BayesMaxPNormal > 0) {  # mcmc_obj
          mcmc_obj <- readRDS(reload_info$expected_file_names[[i]])
          chk.args <- .compare_args(
            current_args = infercnv_obj@options,
            relevant_args = unlist(reload_info$relevant_args[1:i]),
            loaded_options = mcmc_obj@options
          )
          if (chk.args) {
        ...
        chk.args <- .compare_args(
          current_args = infercnv_obj@options,
          relevant_args = unlist(reload_info$relevant_args[1:i]),
          loaded_options = hmm.infercnv_obj@options
        )
        if (chk.args) {
          hmm.infercnv_obj@options <- infercnv_obj@options
          # max for case where step 18 is found, so can be skipped,
          # but still needed to reload step 17 results to be able to run step 19
          skip_hmm <- max(skip_hmm, i - 16)
        ...
        if (skip_past > i) { # in case denoise was found
          ...
          # if 22 denoise found, don't check 21 maskDE
          next
        }
        chk.args <- .compare_args(
          current_args = infercnv_obj@options,
          relevant_args = unlist(reload_info$relevant_args[1:i]),
          loaded_options = reloaded_infercnv_obj@options
        )
        if (!chk.args) {
        ...
        if (i ==  15 && per_chr_hmm_subclusters) {
          ...
          tmp <- paste0(
            substr(reload_info$expected_file_names[[i]], 1, tmp - 13),
            ".per_chr_subclusters",
            substr(reload_info$expected_file_names[[i]], tmp - 12, tmp)
          )
          if (!file.exists(tmp)){
            next
          }
        ...
        # Replace input infercnv_obj
        infercnv_obj <- reloaded_infercnv_obj
        # Remove first (temporary) reference so there's no duplication
        ...
        # Do not stop check right away if denoise was found to
        # also allow to check for HMM
        if (i < 21) {
          break
        }
      }
  ...
  .mylog("incoming data", step_count)
  ...
  .mylog("Removing lowly expressed genes", step_count)

  ## Remove genes that aren't sufficiently expressed, according to min mean
  ## count cutoff. Examines the original (non-log-transformed) data, gets mean
  ## for each gene, and removes genes with mean values below cutoff.
  if (skip_past < step_count) {
    infercnv_obj <- require_above_min_mean_expr_cutoff(
      infercnv_obj = infercnv_obj,
      min_mean_expr_cutoff = cutoff
    )
    ## require each gene to be present in a min number of cells for ref sets
    infercnv_obj <- require_above_min_cells_ref(
      infercnv_obj = infercnv_obj,
  ...
  flog.info(sprintf(fmt, step_count))

  resume_file_token <- ifelse((HMM), paste0("HMM", HMM_type), "")
  if (skip_past < step_count) {
    infercnv_obj <- normalize_counts_by_seq_depth(
      infercnv_obj = infercnv_obj,
      normalize_factor = normalize_factor
    )
    if (HMM && HMM_type == 'i6') {
      ## add in the hidden spike needed by the HMM
      infercnv_obj <- .build_and_add_hspike(
        infercnv_obj = infercnv_obj,
        sim_method = sim_method,
        aggregate_normals = hspike_aggregate_normals
      )
      if (sim_foreground) {
        infercnv_obj <- .sim_foreground(
          infercnv_obj = infercnv_obj,
          sim_method = sim_method
        )
      }
  ...
  .mylog("log transformation of data", step_count)
  ...
  if (scale_data) {
    .mylog("scaling all expression data", step_count)
    if (skip_past < step_count) {
  ...
  if (!is.null(num_ref_groups)) {
    if (!has_reference_cells(infercnv_obj)) {
      stop(
        "Error, no reference cells defined. ",
        "Cannot split them into groups as requested"
      )
    }
    flog.info(sprintf("\n\n\tSTEP %02d: splitting reference data into %d clusters\n", step_count, num_ref_groups))

    if (skip_past < step_count) {
      infercnv_obj <- split_references(
        infercnv_obj = infercnv_obj,
  ...
  if (analysis_mode == 'subclusters' & tumor_subcluster_partition_method == 'random_trees') {
    ...
    if (skip_past < step_count) {
      infercnv_obj <- define_signif_tumor_subclusters_via_random_smooothed_trees(
        infercnv_obj = infercnv_obj,
  ...
  if (skip_past < step_count) {
    infercnv_obj <- subtract_ref_expr_from_obs(
      infercnv_obj = infercnv_obj,
      inv_log = FALSE,
      use_bounds = ref_subtract_use_mean_bounds
    )
    if (save_rds) {
      saveRDS(infercnv_obj, reload_info$expected_file_names[[step_count]])
    }
  ...
  if (skip_past < step_count) {
    if (smooth_method == 'runmeans') {
      ...
      infercnv_obj <- smooth_by_chromosome(
        infercnv_obj = infercnv_obj,
...
#' @noRd
#'
subtract_ref_expr_from_obs <- function(
    infercnv_obj,
    inv_log = FALSE,
    use_bounds = TRUE
) {
  ## r = genes, c = cells
  flog.info(sprintf(
    "::subtract_ref_expr_from_obs:Start inv_log=%s, use_bounds=%s",
    inv_log, use_bounds
  ))
  if (has_reference_cells(infercnv_obj)) {
    ...
    ref_groups <- list(
      'proxyNormal' = unlist(infercnv_obj@observation_grouped_cell_indices)
    )
    flog.info("-no reference cells specified... using mean of all cells as proxy")
  }
  ref_grp_gene_means <- .get_normal_gene_mean_bounds(
    expr.data = infercnv_obj@expr.data,
    ref_groups = ref_groups,
    inv_log = inv_log
  )
  infercnv_obj@expr.data <- .subtract_expr(
    expr_matrix = infercnv_obj@expr.data,
    ref_grp_gene_means = ref_grp_gene_means,
    use_bounds = use_bounds
  )
  if (!is.null(infercnv_obj@.hspike)) {
    flog.info("-mirroring for hspike")
    infercnv_obj@.hspike <- subtract_ref_expr_from_obs(
...
.get_normal_gene_mean_bounds <- function(
    expr.data,
    ref_groups,
    inv_log = FALSE
) {
  if (inv_log) {
    if (inherits(expr.data, "dMatrix")) {
      expr.data@x <- 2 ^ expr.data@x - 1
    } else if (inherits(expr.data, "matrix")) {
      expr.data <- 2 ^ expr.data - 1
    } else {
      stop("`expr.data` only support 'matrix' or 'dMatrix'.")
    }
  }
  gene_ref_grp_means <- matrix(
    data = 0,
    nrow = nrow(expr.data),
    ncol = length(ref_groups)
  )
  rownames(gene_ref_grp_means) <- rownames(expr.data)
  colnames(gene_ref_grp_means) <- names(ref_groups)
  for (i in seq_along(ref_groups)) {
    gene_ref_grp_means[, i] <- Matrix::rowMeans(
      expr.data[, ref_groups[[i]], drop = FALSE]
    )
  }
  if (inv_log) {
    gene_ref_grp_means <- log2(gene_ref_grp_means + 1)
  }
  gene_ref_grp_means
}
...
.subtract_expr <- function(
  ...
  if (use_bounds) {
    subtr_data <- matrix(0, nrow = nrow(expr_matrix), ncol = ncol(expr_matrix))
    rownames(subtr_data) <- rownames(expr_matrix)
    colnames(subtr_data) <- colnames(expr_matrix)

    grp_min <- MatrixGenerics::rowMins(ref_grp_gene_means)
    grp_max <- MatrixGenerics::rowMaxs(ref_grp_gene_means)
    row_init <- rep(0, ncol(expr_matrix))
    for (i in seq_len(nrow(expr_matrix))) {
      x <- as.numeric(expr_matrix[i, , drop = TRUE])
      row_init2 <- row_init
      above_max <- which(x > grp_max[i])
      below_min <- which(x < grp_min[i])
      row_init2[above_max] <- x[above_max] - grp_max[i]
      row_init2[below_min] <- x[below_min] - grp_min[i]
      ## note, in-between values are left at zero!
      subtr_data[i, ] <- row_init2
    }
    return(subtr_data)
  }
  gene_means_mean <- Matrix::rowMeans(ref_grp_gene_means)
  subtr_data <- as.matrix(expr_matrix) - gene_means_mean
  return(subtr_data)
}
...
.center_columns <- function(expr_data, method) {
  ...
  if (method == "median") {
    center <- MatrixGenerics::colMedians(expr_data, na.rm = TRUE)
  } else {
    # by mean
    center <- MatrixGenerics::colMeans(expr_data, na.rm = TRUE)
  }
  Matrix::t(apply(expr_data, 1, "-", center))
}
...
#' @noRd
#'
require_above_min_mean_expr_cutoff <- function(
    infercnv_obj,
    min_mean_expr_cutoff
) {
  flog.info(paste("::above_min_mean_expr_cutoff:Start", sep = ""))
  indices <- .below_min_mean_expr_cutoff(
    expr_data = infercnv_obj@expr.data,
    min_mean_expr = min_mean_expr_cutoff
  )
  if (length(indices) > 0) {
    flog.info(sprintf(
...
.below_min_mean_expr_cutoff <- function(expr_data, min_mean_expr) {
  average_gene <- Matrix::rowMeans(expr_data)
...
require_above_min_cells_ref <- function(infercnv_obj, min_cells_per_gene) {
  n_cells <- Matrix::rowSums(infercnv_obj@expr.data > 0, na.rm = TRUE)
  genes_remove <- which(n_cells < min_cells_per_gene)

  num_genes_total <- dim(infercnv_obj@expr.data)[1]
  num_removed <- length(genes_remove)
  if (num_removed > 0) {
    flog.info(sprintf(
      "Removed %d genes having fewer than %d min cells per gene = %g %% genes removed here",
      num_removed, min_cells_per_gene, num_removed / num_genes_total * 100
    ))
    if (num_removed == num_genes_total) {
      flog.warn("::All genes removed! Must revisit your data..., cannot continue here.")
      stop(998)
    }
    infercnv_obj <- remove_genes(infercnv_obj, genes_remove)
  } else {
...
smooth_by_chromosome <- function(infercnv_obj, window_length, smooth_ends = TRUE) {
  ...
  for (chr in chrs) {
    ...
    if (nrow(chr_data) > 1) {
      smoothed_chr_data = .smooth_window(
        data = chr_data,
        window_length = window_length
      )
...
.smooth_helper <- function(obs_data, window_length) {
  ...
  # defining the iteration range in cases where the window size is larger
  # than the number of genes. In that case we only iterate to the half
  # since the process is applied from both ends.
...
.get_average_bounds <- function(expr_matrix) {
  lower_bound <- mean(MatrixGenerics::colMins(expr_matrix, na.rm = TRUE))
  upper_bound <- mean(MatrixGenerics::colMaxs(expr_matrix, na.rm = TRUE))
...
.normalize_data_matrix_by_seq_depth <- function(
    counts.matrix,
    normalize_factor = NA
) {
  flog.info("normalizing counts matrix by depth")
  data <- counts.matrix
  cs <- Matrix::colSums(data)
  ## make fraction of total counts:
  if (inherits(data, "CsparseMatrix")) {
    data <- data %*% Matrix::Diagonal(x = 1 / cs, names = colnames(data))
  ...
  if (is.na(normalize_factor)) {
    ...
    flog.info(sprintf(
      "Computed total sum normalization factor as median libsize: %f",
      normalize_factor
    ))
  } else {
    flog.info(sprintf(
      "Using specified normalization factor: %f",
      normalize_factor
    ))
  }
  if (is.na(normalize_factor)) {
...
.compare_args <- function(current_args, relevant_args, loaded_options) {
  ...
  all(sapply(
    X = relevant_args[relevant_args %in% names(current_args)],
    FUN = function(arg_n) {
...
.get_relevant_args_list <- function(
  ...
  # creation args
  ## add check for matrix size, col/row names, and ref/obs indices
  ...
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf("%02d_incoming_data.infercnv_obj", step_i)
  )
  step_i <- step_i + 1
  ...
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf("%02d_reduced_by_cutoff.infercnv_obj", step_i)
  )
  step_i <- step_i + 1
  ...
  resume_file_token <- ifelse(
    (run_arguments$HMM),
    paste0("HMM", run_arguments$HMM_type),
    ""
  )
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf(
      "%02d_normalized_by_depth%s.infercnv_obj",
      step_i,
      resume_file_token
    )
  )
  ...
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf("%02d_logtransformed%s.infercnv_obj", step_i, resume_file_token)
  )
  step_i <- step_i + 1
  ...
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf("%02d_scaled%s.infercnv_obj", step_i, resume_file_token)
  )
  step_i <- step_i + 1
  ...
  if (!is.null(run_arguments$num_ref_groups)) {
    ...
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(
        "%02d_split_%sf_refs%s.infercnv_obj",
        step_i,
        resume_file_token,
        run_arguments$num_ref_groups
      )
    )
  }
  step_i <- step_i + 1
  ...
  if (run_arguments$analysis_mode == 'subclusters') {
    if (run_arguments$tumor_subcluster_partition_method == 'random_trees') {
      resume_file_token <- paste0(resume_file_token, ".rand_trees")
      relevant_args[[step_i]] <- c(
        relevant_args[[step_i]],
        "hclust_method",
      ...
      expected_file_names[[step_i]] <- file.path(
        run_arguments$out_dir,
        sprintf(
          "%02d_tumor_subclusters%s.%s.infercnv_obj",
          step_i,
          resume_file_token,
          run_arguments$tumor_subcluster_partition_method
        )
      )
    }
  }
  ...
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf(
      "%02d_remove_ref_avg_from_obs_logFC%s.infercnv_obj",
      step_i, resume_file_token
    )
  )
  step_i <- step_i + 1
  ...
  if (!is.na(run_arguments$max_centered_threshold)) {
    ...
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(
        "%02d_apply_max_centered_expr_threshold%s.infercnv_obj",
        step_i,
        resume_file_token
      )
    )
  ...
  fmt <- "%02d_smoothed_by_chr%s.infercnv_obj"
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf(fmt, step_i, resume_file_token)
  )
  step_i <- step_i + 1

  # 11 _recentered_cells_by_chr%s.infercnv_obj
  relevant_args[[step_i]] <- c()
  fmt <- "%02d_recentered_cells_by_chr%s.infercnv_obj"
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf(fmt, step_i, resume_file_token)
  )
  ...
  relevant_args[[step_i]] <- c("ref_subtract_use_mean_bounds")
  fmt <- "%02d_remove_ref_avg_from_obs_adjust%s.infercnv_obj"
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf(fmt, step_i, resume_file_token)
  )
  ...
  if (run_arguments$remove_genes_at_chr_ends == TRUE) {
    ...
    fmt <- "%02d_remove_gene_at_chr_ends%s.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, resume_file_token)
    )
  ...
  relevant_args[[step_i]] <- c()
  fmt <- "%02d_invert_log_transform%s.infercnv_obj"
  expected_file_names[[step_i]] <- file.path(
    run_arguments$out_dir,
    sprintf(fmt, step_i, resume_file_token)
  )
  ...
  if (run_arguments$analysis_mode != 'subclusters') {
    ...
    fmt <- "%02d_no_subclustering%s.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, resume_file_token)
    )
  ...
  if (run_arguments$analysis_mode == 'subclusters') {
    if (run_arguments$tumor_subcluster_partition_method != 'random_trees') {
      ...
      relevant_args[[step_i]] <- c(
        ...
        "leiden_function_per_chr", "leiden_resolution_per_chr",
        "per_chr_hmm_subclusters",
        "per_chr_hmm_subclusters_references",
        "hclust_method",
        "cluster_by_groups"
      )
      fmt <- "%02d_tumor_subclusters%s.infercnv_obj"
      expected_file_names[[step_i]] <- file.path(
        run_arguments$out_dir,
        sprintf(fmt, step_i, resume_file_token)
      )
  ...
  if (run_arguments$prune_outliers) {
    ...
    fmt <- "%02d_remove_outlier%s.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, resume_file_token)
    )
  ...
  if (run_arguments$HMM) {
    ...
    hmm_resume_file_token <- paste0(
      resume_file_token,
      ".hmm_mode-",
      run_arguments$analysis_mode
    )
    # hmm.infercnv_obj_file <- file.path(
    #   run_arguments$out_dir,
    #   sprintf("%02d_HMM_pred%s.infercnv_obj", step_i, hmm_resume_file_token)
    # )
    if (run_arguments$analysis_mode == 'subclusters') {
      if (run_arguments$tumor_subcluster_partition_method == 'random_trees') {
        relevant_args[[step_i]] <- c(relevant_args[[step_i]], "hclust_method")
    ...
    fmt <- "%02d_HMM_pred%s.infercnv_obj"
    expected_file_names[[step_i]] <- hmm.infercnv_obj_file <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, hmm_resume_file_token)
    )
  }
  step_i <- step_i + 1
  ...
  if (run_arguments$HMM == TRUE & run_arguments$BayesMaxPNormal > 0) {
    ...
    fmt <- "%02d_HMM_pred.Bayes_Net%s.mcmc_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, hmm_resume_file_token)
    )
  ...
  if (run_arguments$HMM == TRUE & run_arguments$BayesMaxPNormal > 0) {
    ...
    # mcmc.infercnv_obj_file = file.path(run_arguments$out_dir, sprintf("%02d_HMM_pred.Bayes_Net%s.Pnorm_%g.infercnv_obj", step_i, hmm_resume_file_token, BayesMaxPNormal))
    # expected_file_names[[step_i]] <- file.path(run_arguments$out_dir, sprintf("%02d_HMM_pred.Bayes_Net%s.mcmc_obj", step_i, hmm_resume_file_token)),
    fmt <- "%02d_HMM_pred.Bayes_Net%s.Pnorm_%g.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, hmm_resume_file_token, run_arguments$BayesMaxPNormal)
    )
  }
  step_i <- step_i + 1
  ...
  if (run_arguments$HMM) {
    ...
    fmt <- "%02d_HMM_pred.repr_intensities%s.Pnorm_%g.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, hmm_resume_file_token, run_arguments$BayesMaxPNormal)
    )
  ...
  if (run_arguments$mask_nonDE_genes) {
    ...
    fmt <- "%02d_mask_nonDE%s.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(fmt, step_i, resume_file_token)
    )
  ...
  if (run_arguments$denoise) {
    ...
    fmt <- "%02d_denoise%s.NF_%s.SD_%g.NL_%s.infercnv_obj"
    expected_file_names[[step_i]] <- file.path(
      run_arguments$out_dir,
      sprintf(
        fmt,
        step_i,
        resume_file_token,
        run_arguments$noise_filter,
        run_arguments$sd_amplifier,
        run_arguments$noise_logistic
      )
    )
  }
  step_i <- step_i + 1

  list(relevant_args = relevant_args, expected_file_names = expected_file_names)
}
